...
BEGIN
    ...
    */
    ...
    SELECT @SourceServerName = 'SQL_' + CASE WHEN CHARINDEX('\', dfd.SourceServerName) > 0 THEN SUBSTRING(dfd.SourceServerName, 1, CHARINDEX('\', dfd.SourceServerName) -1 ) ELSE '' END + '_' + dfd.SourceDatabaseName
	   ...
	   , @CustomWhereStmt = ts.CustomWhereStmt
    ...
    /* case when user set delta load but by default is no adequate column then 1=1 */

    IF (@DeltaColumnName = '1')
    BEGIN 
    SELECT @DeltaColumnType = 'integer'
	   , @DeltaColumnPrecision = 10
	   , @DeltaColumnScale = 0
    END 
    ...
    SELECT @SQL = @SQL 
	   + CASE 
			 WHEN @Pattern = 'Full' 
			 THEN N'<TABLEATTRIBUTE NAME ="Source Filter" VALUE =""/>'
			  
			 WHEN @Pattern = 'Increment' AND @CustomWhereStmt IS NOT NULL 
			 THEN N'<TABLEATTRIBUTE NAME ="Source Filter" VALUE ="' + @CustomWhereStmt + '"/>'

			 WHEN @Pattern = 'Increment' AND @DeltaColumnType = 'date/time'
			 THEN N'<TABLEATTRIBUTE NAME ="Source Filter" VALUE ="' + @DeltaColumnName + ' &gt; CONVERT(DATETIME, ''$$v_Last' + @DeltaColumnName + ''', 121)"/>' /* TODO: obsluga typu timestamp na podstawie @DeltaColumnType */

			 WHEN @Pattern = 'Increment' 
			 THEN N'<TABLEATTRIBUTE NAME ="Source Filter" VALUE ="' + @DeltaColumnName + ' &gt; $$v_Last' + @DeltaColumnName + '"/>' /* TODO: obsluga typu timestamp na podstawie @DeltaColumnType */
	   END
    ...
    --SELECT @sql, len(@sql), @INF_TableName, @Pattern , @TableName, @TargetSchema, @SourceSchema, @INF_TableName, @SourceDBName, @DeltaColumnName, @DeltaColumnType, @DeltaColumnPrecision, @DeltaColumnScale

    SELECT @SQL = @SQL 
	   + CASE 
			 WHEN @Pattern = 'Increment' AND @DeltaColumnType = 'date/time'
			 THEN N'<TRANSFORMFIELD DATATYPE ="string" DEFAULTVALUE ="" DESCRIPTION ="" '
				+ 'EXPRESSION ="SETMAXVARIABLE($$v_Last' + @DeltaColumnName + ', (TO_CHAR(' + @DeltaColumnName + ', ''YYYY-MM-DD HH24:MI:SS.US'')))" EXPRESSIONTYPE ="GENERAL" NAME ="v_Last' + @DeltaColumnName + '" PICTURETEXT ="" PORTTYPE ="LOCAL VARIABLE" PRECISION ="10" SCALE ="0"/>'

			 WHEN @Pattern = 'Increment' 
			 THEN N'<TRANSFORMFIELD DATATYPE ="' + @DeltaColumnType + '" DEFAULTVALUE ="" DESCRIPTION ="" '
    ...
    SELECT @SQL = @SQL 
	   + CASE 
			 WHEN @Pattern = 'Increment' AND @DeltaColumnType = 'date/time'
			 THEN N'<MAPPINGVARIABLE AGGFUNCTION ="MAX" DATATYPE ="string" DEFAULTVALUE ="1900-01-01 00:00:00.000" DESCRIPTION ="" ISEXPRESSIONVARIABLE ="NO" ISPARAM ="NO" NAME ="$$v_Last' + cast(@DeltaColumnName as varchar) + '" PRECISION ="23" SCALE ="0" USERDEFINED ="YES"/>' + nchar(10)

			 WHEN @Pattern = 'Increment' 
			 THEN N'<MAPPINGVARIABLE AGGFUNCTION ="MAX" DATATYPE ="' + @DeltaColumnType + '" DEFAULTVALUE ="0" DESCRIPTION ="" ISEXPRESSIONVARIABLE ="NO" ISPARAM ="NO" NAME ="$$v_Last' + cast(@DeltaColumnName as varchar) + '" PRECISION ="' + cast(@DeltaColumnPrecision as varchar) + '" SCALE ="' + cast(@DeltaColumnScale as varchar) + '" USERDEFINED ="YES"/>' + nchar(10)
	   END
