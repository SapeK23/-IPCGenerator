...
BEGIN
    ...
    */
    ...
    SET @SQL = @SQL + nchar(10) + N'<SOURCE BUSINESSNAME ="" DATABASETYPE ="' + @SourceDBType + '" DBDNAME ="' + @SourceServerName + '" DESCRIPTION ="" NAME ="' + @INF_TableName + '" OBJECTVERSION ="1" OWNERNAME ="' + @SchemaName + '" VERSIONNUMBER ="1">'
        
    SET @set = ''
    SELECT @set = @set 
	   + '<SOURCEFIELD BUSINESSNAME ="" DATATYPE ="' + m.TypeName + '" ' 
	   ...
	   + 'PHYSICALLENGTH ="' + CASE WHEN m.Precision = 0 THEN CAST(m.MaxLenght AS VARCHAR) ELSE CAST(m.Precision AS VARCHAR) END + '" PHYSICALOFFSET ="0" PICTURETEXT ="" ' 
	   + 'PRECISION ="'+ CASE WHEN m.Precision = 0 THEN CAST(m.MaxLenght AS VARCHAR) ELSE CAST(m.Precision AS VARCHAR) END + '" SCALE ="' + CAST(m.Scale AS VARCHAR)+'" USAGE_FLAGS =""/>'
	   + nchar(10)
    FROM ipc.Metadata m
    WHERE 1=1
